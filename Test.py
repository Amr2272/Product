...
def run_dashboard(train, min_date, max_date, sort_state):
    ...
    st.markdown("---")
    col_chosen = st.multiselect("Choose State", options=sort_state.index.tolist(), default=['Pichincha'] if 'Pichincha' in sort_state.index else (sort_state.index.tolist()[:1] if not sort_state.empty else []), key='state_id_dash', placeholder="Select one or more states")
    agg_method = st.radio("Aggregate Method", options=['sum', 'mean'], index=0, horizontal=True, key='value_id_dash')
    st.markdown("---")
    date_range = st.date_input("Select Date Range", value=[min_date, max_date], min_value=date(2000, 1, 1), max_value=date(2100, 1, 1), key='date_id_dash')
    if len(date_range) == 2:
    ...
    try:
        mask = ((train['state'].isin(col_chosen)) & (train.index >= start_date) & (train.index <= end_date))
        filterr = train[mask]
        if filterr.empty:
            ...
            fig = px.bar(city_sales, x='city', y='sales', labels={'city': 'City', 'sales': f'Sales ({agg_method.capitalize()})'}, title=f'City Sales for Selected States ({agg_method.capitalize()})', text=city_sales['sales'].apply(lambda x: f'{x:,.0f}'), color_discrete_sequence=["#1abc9c"])
...
def check_model_performance(prophet_df, forecast_data, mae_percent_threshold=0.25):
    ...
    if not performance_df.empty:
        ...
        alert_status = "ALERT: High MAE" if mae > threshold_value else "Performance OK"
...
def run_forecast_app(model, prophet_df):
    ...
    if st.sidebar.button("🚀 Run Forecast", key='forecast_button') and periods > 0:
    ...
    st.subheader("📤 Batch Forecast (Upload CSV)")
    uploaded_file = st.file_uploader("Upload a CSV file with dates", type=["csv"])
    if uploaded_file is not None:
        batch_df = pd.read_csv(uploaded_file)
        if 'ds' not in batch_df.columns:
            st.error("CSV must contain a 'ds' column with dates.")
        else:
            batch_df['ds'] = pd.to_datetime(batch_df['ds'])
            forecast_batch = model.predict(batch_df)
            st.success("Batch forecast generated successfully!")
            st.dataframe(forecast_batch[['ds','yhat','yhat_lower','yhat_upper']].rename(columns={'ds':'Date','yhat':'Forecast','yhat_lower':'Lower Bound','yhat_upper':'Upper Bound'}).set_index('Date'))
            st.download_button(label="Download Batch Forecast as CSV", data=forecast_batch.to_csv(index=False), file_name='batch_forecast.csv', mime='text/csv')
